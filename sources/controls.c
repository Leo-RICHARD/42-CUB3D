...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/29 21:24:02 by lrichard          #+#    #+#             */
/*   Updated: 2021/10/04 18:05:17 by lrichard         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (inputs->mvforward && \
			!segfaults_forward(args, inputs, player->pos))
	{
		player->pos[0] = player->pos[2];
		player->pos[1] = player->pos[3];
	}
	if (inputs->mvleft && \
			!segfaults_left(args, inputs, player->pos))
	{
		player->pos[0] = player->pos[2];
		player->pos[1] = player->pos[3];
	}
	if (inputs->mvbackward && \
			!segfaults_backward(args, inputs, player->pos))
	{
		player->pos[0] = player->pos[2];
		player->pos[1] = player->pos[3];
	}
	if (inputs->mvright && \
			!segfaults_right(args, inputs, player->pos))
	{
		player->pos[0] = player->pos[2];
		player->pos[1] = player->pos[3];
	}
